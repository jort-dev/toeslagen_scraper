...
from test.test_gegevens import *
...
async def run_playwright():
    async with async_playwright() as p:
        ...
        if str(demo_checkbox_var.get()) == "1":
            ...
            await vul_uitgebreide_test_gegevens_in(page)  # TEST
        ...
        while True:
            ...
            count = 1
            while True:
                id_true = f"label[for='V6-24-{count}_pbt_True']"
                id_false = f"label[for='V6-24-{count}_pbt_False']"
                count += 1
                selected_text = selected_option.get()
                selected_index = kind_opties.index(selected_text)
                if selected_index == 0:  # geen kind
                    break

                await asyncio.sleep(0.1)
                if selected_index == 1:  # thuiswonend
                    # print(f"Kiezen: thuiswonend")
                    locator = page.locator(id_true)
                elif selected_index == 2:  # uitwonend
                    # print(f"Kiezen: uitwonend")
                    locator = page.locator(id_false)
...
kind_opties = [
    ...
    "Mijn kind woont thuis",
    "Mijn kind woont niet thuis"
]

# Create a StringVar to hold the selected option
selected_option = tk.StringVar(value=kind_opties[0])  # Default value
...
Hovertip(combobox, "Als je kinderen hebt, wordt deze vraag opnieuw gesteld wanneer het script het inkomen verhoogd", hover_delay=tooltip_delay)
...
# demo mode
demo_checkbox_var = tk.IntVar(value=0)
